...
namespace PriceModifierPipeline.Tests
{
    public class DirectPriceTests
    {
            ...
            SeasonPriceState season, DistancePriceState distance, TradeEventState tradeEvent, int expected)
        {
            ...
            var preview = new DirectPreviewController().CalculatePreview(input);
            var commit = new DirectCommitController().CalculateCommit(input);
        ...
        public void MidpointRoundsAwayFromZero()
        {
            ...
            Assert.That(new DirectPreviewController().CalculatePreview(input).FinalPrice, Is.EqualTo(2));
            Assert.That(new DirectCommitController().CalculateCommit(input).FinalPrice, Is.EqualTo(2));
        }
        ...
        public void OverflowThrowsInBothPaths()
        {
            var input = new SellPriceInput(int.MaxValue, TradeItemType.Normal, SeasonPriceState.Favored,
                DistancePriceState.Long, TradeEventState.Lucky);
            Assert.Throws<OverflowException>(() => new DirectPreviewController().CalculatePreview(input));
            Assert.Throws<OverflowException>(() => new DirectCommitController().CalculateCommit(input));
        }
        ...
        public void CommandsInvalidateBothResultsAndResetRestoresDefaults()
        {
            ...
            try
            {
                var runtime = owner.AddComponent<DirectPriceRuntime>();
                var source = owner.AddComponent<DirectPriceDebugSource>();
                var command = owner.AddComponent<DirectPriceDebugCommand>();
